...
class LearningProfileExtractor:
    def extract_from_signals(self,
                            profile: LearningProfile,
                            request: ConversationAnalysisRequest, 
                            extracted_signals: list[ExtractedLearningSignal]):
        ...
        for grouped_key,topic_signals in grouped_signals.items():
            ...
            low_confidence=any(item.signal_type=="confidence" and 
                               (item.confidence_label=="low" or item.confidence_score < 0.35) 
                               for item in topic_signals)
            high_confidence=any(item.signal_type=="confidence" and 
                                (item.confidence_label=="high" or item.confidence_score > 0.7) 
                                for item in topic_signals)
            ...
            weakness_detected = any(item.signal_type in ("weakness", "mistake") 
                                for item in topic_signals) or incorrect_detected or bool(mistakes)
            strength_detected=any(item.signal_type=="strength" 
                                for item in topic_signals)or(correct_detected and not weakness_detected)
            signal.attempts+=1
            if correct_detected:
            ...
            self.apply_confidence(signal,
                                low_confidence,
                                high_confidence,
                                correct_detected if correct_detected or incorrect_detected else None)
            self.apply_speed(signal,
                             slow_signal,
                             fast_signal,
            ...
            memory_updates.extend(self.build_memory_updates(signal,
                                                        profile,
                                                        mistakes,
                                                        strength_detected,
                                                        weakness_detected,
        ...
        student_info_update=StudentInfoUpdateDraft(learning_style=profile.preferred_learning_style,
                                                strong_points=profile.strengths,
                                                weak_points=profile.weaknesses)
    ...
    def store_extracted_evidence(self,
                                signal:TopicLearningSignal,
                                topic_signals:list[ExtractedLearningSignal],
                                request:ConversationAnalysisRequest):
    ...
    def apply_strength_or_weakness(self,
                                signal:TopicLearningSignal,
    ...
    def refresh_profile_rollups(self,profile:LearningProfile):
        ...
        profile.strengths = sorted({signal.topic for signal in topic_values 
                                if self.topic_mastery_score(signal)>=70 and signal.weakness_score<0.45})
        profile.weaknesses = sorted({signal.topic for signal in topic_values 
                                if self.topic_mastery_score(signal)<=55 or signal.weakness_score>=0.2})
    ...
    def build_memory_updates(self,
                            signal:TopicLearningSignal,
                            profile:LearningProfile,
                            mistakes:list[str],
                            strength_detected:bool,
                            weakness_detected:bool,
                            style_signal:str|None):
        updates=[]
        if weakness_detected:
            detail=f"Student showed weakness in {signal.topic}; mistakes: {', '.join(mistakes) if mistakes else 'model detected weakness'}."
            updates.append(MemoryAdapterEntry(memory_type="weakness",
                                            subject=signal.subject,
                                            topic=signal.topic,
                                            detail=detail,
                                            confidence=confidence_label(signal.confidence_score)))
        if strength_detected:
            detail=f"Student showed strength in {signal.topic}."
            updates.append(MemoryAdapterEntry(memory_type="strength",
                                              subject=signal.subject,
                                              topic=signal.topic,
                                              detail=detail,
                                              confidence=confidence_label(signal.confidence_score)))
        if style_signal is not None:
            ...
            updates.append(MemoryAdapterEntry(memory_type="learning_style",
                                            subject=signal.subject,
                                            topic=signal.topic,
                                            detail=detail,
    ...
    def build_semantic_events(self,
                            signal:TopicLearningSignal,
                            profile:LearningProfile,
                            strength_detected:bool,
                            weakness_detected:bool):
